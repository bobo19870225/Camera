...
public class CollageTwentyTwo extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyTwo$3 */
    class C07993 implements Runnable {
        C07993() {
        }

        public void run() {
            CollageTwentyTwo.this.setGridPadding(CollageTwentyTwo.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyTwo$4 */
    class C08004 extends SimpleOnGestureListener {
        C08004() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageTwentyTwo.this.mPopupWindow != null) {
                if (CollageTwentyTwo.this.mPopupWindow.isShowing()) {
                    CollageTwentyTwo.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageTwentyTwo.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageTwentyTwo.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyTwo$5 */
    class C08015 implements OnClickListener {
        C08015() {
        }

        public void onClick(View view) {
            CollageTwentyTwo.this.mPopupWindow.dismiss();
            CollageTwentyTwo.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyTwo$6 */
    class C08026 implements OnClickListener {
        C08026() {
        }

        public void onClick(View view) {
            CollageTwentyTwo.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageTwentyTwo.this.getStoredBitmap(CollageTwentyTwo.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageTwentyTwo.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageTwentyTwo.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyTwo$7 */
    class C08037 implements OnClickListener {
        C08037() {
        }
    ...
    public static CollageTwentyTwo newInstance(boolean b) {
        CollageTwentyTwo fragment = new CollageTwentyTwo();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_22, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageTwentyTwo.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyTwo$2$1 */
                class C07971 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageTwentyTwo.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageTwentyTwo.this.getActivity().runOnUiThread(new C07971());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        ImageUtils.optimizeView(this.rl7, (float) (Dimensions.S_3P * 2), 0.0f, Dimensions.S_3P, Dimensions.MATCH_PARENT);
        view.post(new C07993());
    }
                ...
                if (view.getId() != R.id.div1 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl2.getY() + ((float) this.rl2.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                    if (view.getId() != R.id.div2 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl4.getX() + ((float) this.rl4.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                        if (view.getId() != R.id.div3 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.div4.getX() + ((float) this.div4.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                            if (view.getId() == R.id.div4 && (view.getX() - ((float) dx)) + ((float) view.getWidth()) <= this.rl7.getX() + ((float) this.rl7.getWidth()) && view.getX() - ((float) dx) >= this.div3.getX()) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C08004());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C08015());
        delete.setOnClickListener(new C08026());
        cancel.setOnClickListener(new C08037());
    }
