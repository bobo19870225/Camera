...
public class CollageThirty extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirty$3 */
    class C06313 implements Runnable {
        C06313() {
        }

        public void run() {
            CollageThirty.this.setGridPadding(CollageThirty.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirty$4 */
    class C06324 extends SimpleOnGestureListener {
        C06324() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageThirty.this.mPopupWindow != null) {
                if (CollageThirty.this.mPopupWindow.isShowing()) {
                    CollageThirty.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageThirty.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageThirty.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirty$5 */
    class C06335 implements OnClickListener {
        C06335() {
        }

        public void onClick(View view) {
            CollageThirty.this.mPopupWindow.dismiss();
            CollageThirty.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirty$6 */
    class C06346 implements OnClickListener {
        C06346() {
        }

        public void onClick(View view) {
            CollageThirty.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageThirty.this.getStoredBitmap(CollageThirty.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageThirty.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageThirty.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirty$7 */
    class C06357 implements OnClickListener {
        C06357() {
        }
    ...
    public static CollageThirty newInstance(boolean b) {
        CollageThirty fragment = new CollageThirty();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_30, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageThirty.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirty$2$1 */
                class C06291 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageThirty.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageThirty.this.getActivity().runOnUiThread(new C06291());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        ImageUtils.optimizeView(this.rl3, 0.0f, 0.0f, Dimensions.S_4P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl4, (float) Dimensions.S_4P, 0.0f, Dimensions.S_4P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl5, (float) (Dimensions.S_4P * 2), 0.0f, Dimensions.S_4P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl6, (float) (Dimensions.S_4P * 3), 0.0f, Dimensions.S_4P, Dimensions.MATCH_PARENT);
        view.post(new C06313());
    }
    ...
    public void setGridPadding(int padding) {
        ...
        this.div2.setX((this.rl3.getX() + ((float) this.rl3.getWidth())) - ((float) (this.curPadding / 2)));
        this.div3.setX((this.rl4.getX() + ((float) this.rl4.getWidth())) - ((float) (this.curPadding / 2)));
        ...
        switch (motionEvent.getAction()) {
            ...
            case 2:
                ...
                if (view.getId() != R.id.div1 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl2.getY() + ((float) this.rl2.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                    if (view.getId() != R.id.div2 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.div3.getX() + ((float) this.div3.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                        if (view.getId() != R.id.div3 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.div4.getX() + ((float) this.div4.getWidth()) || view.getX() - ((float) dx) < this.div2.getX()) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C06324());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C06335());
        delete.setOnClickListener(new C06346());
        cancel.setOnClickListener(new C06357());
    }
