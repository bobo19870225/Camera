...
public class CollageFourty extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFourty$3 */
    class C04793 implements Runnable {
        C04793() {
        }

        public void run() {
            CollageFourty.this.setGridPadding(CollageFourty.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFourty$4 */
    class C04804 extends SimpleOnGestureListener {
        C04804() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageFourty.this.mPopupWindow != null) {
                if (CollageFourty.this.mPopupWindow.isShowing()) {
                    CollageFourty.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageFourty.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageFourty.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFourty$5 */
    class C04815 implements OnClickListener {
        ...
        public void onClick(View view) {
            CollageFourty.this.mPopupWindow.dismiss();
            CollageFourty.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFourty$6 */
    class C04826 implements OnClickListener {
        ...
        public void onClick(View view) {
            CollageFourty.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageFourty.this.getStoredBitmap(CollageFourty.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageFourty.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageFourty.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFourty$7 */
    ...
    public static CollageFourty newInstance(boolean b) {
        CollageFourty fragment = new CollageFourty();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_40, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageFourty.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFourty$2$1 */
                class C04771 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageFourty.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageFourty.this.getActivity().runOnUiThread(new C04771());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        main_rel.invalidate();
        ImageUtils.optimizeView(this.rl1, 0.0f, 0.0f, Dimensions.MATCH_PARENT, Dimensions.S_5P);
        ImageUtils.optimizeView(this.rl2, 0.0f, (float) Dimensions.S_5P, Dimensions.MATCH_PARENT, Dimensions.S_5P);
        ImageUtils.optimizeView(this.rl3, 0.0f, (float) (Dimensions.S_5P * 2), Dimensions.MATCH_PARENT, Dimensions.S_5P);
        ImageUtils.optimizeView(this.rl4, 0.0f, (float) (Dimensions.S_5P * 3), Dimensions.MATCH_PARENT, Dimensions.S_5P);
        ImageUtils.optimizeView(this.rl5, 0.0f, (float) (Dimensions.S_5P * 4), Dimensions.MATCH_PARENT, Dimensions.S_5P);
        view.post(new C04793());
    }
    ...
    public void setGridPadding(int padding) {
        ...
        lp2.setMargins(this.curPadding, this.curPadding / 2, this.curPadding, this.curPadding / 2);
        ...
        lp4.setMargins(this.curPadding, this.curPadding / 2, this.curPadding, this.curPadding / 2);
        LayoutParams lp5 = (LayoutParams) this.fl5.getLayoutParams();
        lp5.setMargins(this.curPadding, this.curPadding / 2, this.curPadding, this.curPadding);
        ...
        this.div4.setY((this.rl4.getY() + ((float) this.rl4.getHeight())) - ((float) (this.curPadding / 2)));
    }
            ...
            case 2:
                ...
                if (view.getId() != R.id.div1 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.div2.getY() + ((float) this.div2.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                    if (view.getId() != R.id.div2 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.div3.getY() + ((float) this.div3.getHeight()) || view.getY() - ((float) dy) < this.div1.getY()) {
                        if (view.getId() != R.id.div3 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.div4.getY() + ((float) this.div4.getHeight()) || view.getY() - ((float) dy) < this.div2.getY()) {
                            if (view.getId() == R.id.div4 && (view.getY() - ((float) dy)) + ((float) view.getHeight()) <= this.rl5.getY() + ((float) this.rl5.getHeight()) && view.getY() - ((float) dy) >= this.div3.getY()) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C04804());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C04815());
        delete.setOnClickListener(new C04826());
        cancel.setOnClickListener(new C04837());
    }
