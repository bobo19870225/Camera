...
public class CollageNineteen extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageNineteen$3 */
    class C05673 implements Runnable {
        C05673() {
        }

        public void run() {
            CollageNineteen.this.setGridPadding(CollageNineteen.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageNineteen$4 */
    class C05684 extends SimpleOnGestureListener {
        C05684() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageNineteen.this.mPopupWindow != null) {
                if (CollageNineteen.this.mPopupWindow.isShowing()) {
                    CollageNineteen.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageNineteen.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageNineteen.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageNineteen$5 */
    class C05695 implements OnClickListener {
        ...
        public void onClick(View view) {
            CollageNineteen.this.mPopupWindow.dismiss();
            CollageNineteen.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageNineteen$6 */
    class C05706 implements OnClickListener {
        ...
        public void onClick(View view) {
            CollageNineteen.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageNineteen.this.getStoredBitmap(CollageNineteen.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageNineteen.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageNineteen.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageNineteen$7 */
    class C05717 implements OnClickListener {
        C05717() {
        }
    ...
    public static CollageNineteen newInstance(boolean b) {
        CollageNineteen fragment = new CollageNineteen();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_19, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageNineteen.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageNineteen$2$1 */
                class C05651 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageNineteen.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageNineteen.this.getActivity().runOnUiThread(new C05651());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        ImageUtils.optimizeView(this.rl3, 0.0f, 0.0f, Dimensions.S_3P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl4, (float) Dimensions.S_3P, 0.0f, Dimensions.S_3P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl5, (float) (Dimensions.S_3P * 2), 0.0f, Dimensions.S_3P, Dimensions.MATCH_PARENT);
        view.post(new C05673());
    }
            ...
            case 2:
                ...
                if (view.getId() != R.id.div1 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl2.getY() + ((float) this.rl2.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                    if (view.getId() != R.id.div2 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.div3.getX() + ((float) this.div3.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                        if (view.getId() == R.id.div3 && (view.getX() - ((float) dx)) + ((float) view.getWidth()) <= this.rl5.getX() + ((float) this.rl5.getWidth()) && view.getX() - ((float) dx) >= this.div2.getX()) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C05684());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C05695());
        delete.setOnClickListener(new C05706());
        cancel.setOnClickListener(new C05717());
    }
