...
public class CollageFifty extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFifty$3 */
    class C04473 implements Runnable {
        C04473() {
        }

        public void run() {
            CollageFifty.this.setGridPadding(CollageFifty.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFifty$4 */
    class C04484 extends SimpleOnGestureListener {
        C04484() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageFifty.this.mPopupWindow != null) {
                if (CollageFifty.this.mPopupWindow.isShowing()) {
                    CollageFifty.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageFifty.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageFifty.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFifty$5 */
    class C04495 implements OnClickListener {
        C04495() {
        }

        public void onClick(View view) {
            CollageFifty.this.mPopupWindow.dismiss();
            CollageFifty.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFifty$6 */
    class C04506 implements OnClickListener {
        C04506() {
        }

        public void onClick(View view) {
            CollageFifty.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageFifty.this.getStoredBitmap(CollageFifty.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageFifty.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageFifty.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFifty$7 */
    ...
    public static CollageFifty newInstance(boolean b) {
        CollageFifty fragment = new CollageFifty();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_50, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageFifty.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageFifty$2$1 */
                class C04451 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageFifty.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageFifty.this.getActivity().runOnUiThread(new C04451());
                    }
                }
            });
            ...
            case 2:
                ...
                if (view.getId() != R.id.div1 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl2.getX() + ((float) this.rl2.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                    if (view.getId() != R.id.div2 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl4.getY() + ((float) this.rl4.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                        if (view.getId() == R.id.div3 && (view.getY() - ((float) dy)) + ((float) view.getHeight()) <= this.rl6.getY() + ((float) this.rl6.getHeight()) && view.getY() - ((float) dy) >= 0.0f) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C04484());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C04495());
        delete.setOnClickListener(new C04506());
        cancel.setOnClickListener(new C04517());
    }
