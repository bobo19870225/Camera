...
public class CollageThirtyThree extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyThree$3 */
    class C06953 implements Runnable {
        C06953() {
        }

        public void run() {
            CollageThirtyThree.this.setGridPadding(CollageThirtyThree.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyThree$4 */
    class C06964 extends SimpleOnGestureListener {
        C06964() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageThirtyThree.this.mPopupWindow != null) {
                if (CollageThirtyThree.this.mPopupWindow.isShowing()) {
                    CollageThirtyThree.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageThirtyThree.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageThirtyThree.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyThree$5 */
    class C06975 implements OnClickListener {
        ...
        public void onClick(View view) {
            CollageThirtyThree.this.mPopupWindow.dismiss();
            CollageThirtyThree.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyThree$6 */
    class C06986 implements OnClickListener {
        C06986() {
        }

        public void onClick(View view) {
            CollageThirtyThree.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageThirtyThree.this.getStoredBitmap(CollageThirtyThree.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageThirtyThree.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageThirtyThree.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyThree$7 */
    class C06997 implements OnClickListener {
        C06997() {
        }
    ...
    public static CollageThirtyThree newInstance(boolean b) {
        CollageThirtyThree fragment = new CollageThirtyThree();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_33, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageThirtyThree.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyThree$2$1 */
                class C06931 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageThirtyThree.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageThirtyThree.this.getActivity().runOnUiThread(new C06931());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        ImageUtils.optimizeView(this.rl8, 0.0f, 0.0f, Dimensions.MATCH_PARENT, Dimensions.S_2P);
        ImageUtils.optimizeView(this.rl9, 0.0f, (float) Dimensions.S_2P, Dimensions.MATCH_PARENT, Dimensions.S_2P);
        view.post(new C06953());
    }
    ...
    public void setGridPadding(int padding) {
        ...
        this.div5.setY((float) (this.rl8.getHeight() - (this.curPadding / 2)));
    }
                    ...
                    if (view.getId() != R.id.div2 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl3.getX() + ((float) this.rl3.getWidth()) || view.getX() - ((float) dx) < this.div1.getX()) {
                        if (view.getId() != R.id.div3 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl5.getY() + ((float) this.rl5.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                            if (view.getId() != R.id.div4 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl7.getY() + ((float) this.rl7.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                                if (view.getId() == R.id.div5 && (view.getY() - ((float) dy)) + ((float) view.getHeight()) <= this.rl9.getY() + ((float) this.rl9.getHeight()) && view.getY() - ((float) dy) >= 0.0f) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C06964());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C06975());
        delete.setOnClickListener(new C06986());
        cancel.setOnClickListener(new C06997());
    }
