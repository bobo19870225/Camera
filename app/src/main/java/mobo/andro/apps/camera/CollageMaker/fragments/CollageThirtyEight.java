...
public class CollageThirtyEight extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyEight$3 */
    class C06393 implements Runnable {
        C06393() {
        }

        public void run() {
            CollageThirtyEight.this.setGridPadding(CollageThirtyEight.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyEight$4 */
    class C06404 extends SimpleOnGestureListener {
        C06404() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageThirtyEight.this.mPopupWindow != null) {
                if (CollageThirtyEight.this.mPopupWindow.isShowing()) {
                    CollageThirtyEight.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageThirtyEight.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageThirtyEight.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyEight$5 */
    class C06415 implements OnClickListener {
        C06415() {
        }

        public void onClick(View view) {
            CollageThirtyEight.this.mPopupWindow.dismiss();
            CollageThirtyEight.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyEight$6 */
    class C06426 implements OnClickListener {
        C06426() {
        }

        public void onClick(View view) {
            CollageThirtyEight.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageThirtyEight.this.getStoredBitmap(CollageThirtyEight.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageThirtyEight.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageThirtyEight.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyEight$7 */
    class C06437 implements OnClickListener {
        C06437() {
        }
    ...
    public static CollageThirtyEight newInstance(boolean b) {
        CollageThirtyEight fragment = new CollageThirtyEight();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_38, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageThirtyEight.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyEight$2$1 */
                class C06371 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageThirtyEight.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageThirtyEight.this.getActivity().runOnUiThread(new C06371());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        main_rel.invalidate();
        ImageUtils.optimizeView(this.rl1, 0.0f, 0.0f, Dimensions.S_3P * 2, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl2, (float) (Dimensions.S_3P * 2), 0.0f, Dimensions.S_3P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl3, 0.0f, 0.0f, Dimensions.MATCH_PARENT, Dimensions.S_3P);
        ImageUtils.optimizeView(this.rl4, 0.0f, (float) Dimensions.S_3P, Dimensions.MATCH_PARENT, Dimensions.S_3P * 2);
        ImageUtils.optimizeView(this.rl5, 0.0f, 0.0f, Dimensions.S_3P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl6, (float) Dimensions.S_3P, 0.0f, Dimensions.S_3P, Dimensions.MATCH_PARENT);
    ...
    public void setGridPadding(int padding) {
        ...
        this.div4.setY((float) (this.rl7.getHeight() - (this.curPadding / 2)));
        this.div5.setY((this.rl8.getY() + ((float) this.rl8.getHeight())) - ((float) (this.curPadding / 2)));
    }
            ...
            case 2:
                ...
                if (view.getId() != R.id.div1 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl2.getX() + ((float) this.rl2.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                    if (view.getId() != R.id.div2 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl4.getY() + ((float) this.rl4.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                        if (view.getId() != R.id.div3 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl6.getX() + ((float) this.rl6.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                            if (view.getId() != R.id.div4 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.div5.getY() + ((float) this.div5.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                                if (view.getId() == R.id.div5 && (view.getY() - ((float) dy)) + ((float) view.getHeight()) <= this.rl9.getY() + ((float) this.rl9.getHeight()) && view.getY() - ((float) dy) >= this.div4.getY()) {
                ...
                if (this.rl1.getLayoutParams().width > this.rl5.getLayoutParams().width) {
                    this.rl6.getLayoutParams().width = this.rl1.getLayoutParams().width - this.rl5.getLayoutParams().width;
                }
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C06404());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C06415());
        delete.setOnClickListener(new C06426());
        cancel.setOnClickListener(new C06437());
    }
