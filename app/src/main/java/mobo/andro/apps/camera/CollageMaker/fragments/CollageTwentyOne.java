...
public class CollageTwentyOne extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyOne$3 */
    class C07673 implements Runnable {
        C07673() {
        }

        public void run() {
            CollageTwentyOne.this.setGridPadding(CollageTwentyOne.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyOne$4 */
    class C07684 extends SimpleOnGestureListener {
        C07684() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageTwentyOne.this.mPopupWindow != null) {
                if (CollageTwentyOne.this.mPopupWindow.isShowing()) {
                    CollageTwentyOne.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageTwentyOne.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageTwentyOne.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyOne$5 */
    class C07695 implements OnClickListener {
        C07695() {
        }

        public void onClick(View view) {
            CollageTwentyOne.this.mPopupWindow.dismiss();
            CollageTwentyOne.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyOne$6 */
    class C07706 implements OnClickListener {
        C07706() {
        }

        public void onClick(View view) {
            CollageTwentyOne.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageTwentyOne.this.getStoredBitmap(CollageTwentyOne.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageTwentyOne.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageTwentyOne.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyOne$7 */
    class C07717 implements OnClickListener {
        C07717() {
        }
    ...
    public static CollageTwentyOne newInstance(boolean b) {
        CollageTwentyOne fragment = new CollageTwentyOne();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_21, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageTwentyOne.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageTwentyOne$2$1 */
                class C07651 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageTwentyOne.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageTwentyOne.this.getActivity().runOnUiThread(new C07651());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        ImageUtils.optimizeView(this.rl7, 0.0f, (float) (Dimensions.S_3P * 2), Dimensions.MATCH_PARENT, Dimensions.S_3P);
        view.post(new C07673());
    }
                ...
                if (view.getId() != R.id.div1 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl2.getX() + ((float) this.rl2.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                    if (view.getId() != R.id.div2 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl4.getY() + ((float) this.rl4.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                        if (view.getId() != R.id.div3 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.div4.getY() + ((float) this.div4.getHeight()) || view.getY() - ((float) dy) < 0.0f) {
                            if (view.getId() == R.id.div4 && (view.getY() - ((float) dy)) + ((float) view.getHeight()) <= this.rl7.getY() + ((float) this.rl7.getHeight()) && view.getY() - ((float) dy) >= this.div3.getY()) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C07684());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C07695());
        delete.setOnClickListener(new C07706());
        cancel.setOnClickListener(new C07717());
    }
