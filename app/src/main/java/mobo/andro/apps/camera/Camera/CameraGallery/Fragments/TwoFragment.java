package mobo.andro.apps.camera.Camera.CameraGallery.Fragments;
...
import mobo.andro.apps.camera.Camera.CameraGallery.Adapters.BucketsAdapter;
import mobo.andro.apps.camera.Camera.CameraGallery.AlbumCustomData;
import mobo.andro.apps.camera.Camera.CameraGallery.OpenGalleryItem;
import mobo.andro.apps.camera.R;

import static mobo.andro.apps.camera.Camera.CameraGallery.OpenGalleryItem.temp_data_open_gallery;
    ...
    private void populateRecyclerView() {
        ...
        recyclerView.addOnItemTouchListener(new RecyclerTouchListener(getContext(), recyclerView, new ClickListener() {
            ...
            public void onClick(View view, int position) {
                ...
                {
                    File file = new File(videosList.get(i));
                    Date lastModDate = new Date(file.lastModified());
                    String[] str = lastModDate.toString().split(" ");
                    temp_date.add(Integer.parseInt(str[2]));
                    albumCustomData.add(new AlbumCustomData(videosList.get(i),str[str.length - 1],str[1],Integer.parseInt(str[2]),false));
                }
...
/*                Intent intent=new Intent(getContext(), OpenGallery.class);
    ...
    public int getVideosSize(String bucket){
        Cursor cursor = getContext().getContentResolver().query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, projection2,                 MediaStore.Video.Media.BUCKET_DISPLAY_NAME+" =?",new String[]{bucket},MediaStore.Video.Media.DATE_ADDED);
        ArrayList<String> imagesTEMP = new ArrayList<>(cursor.getCount());
        HashSet<String> albumSet = new HashSet<>();
        File file;
        if (cursor.moveToLast()) {
        ...
        return imagesTEMP.size();
    }


    public String getVideosBitmap(String bucket) {
        String temp=null;
        Cursor cursor = getContext().getContentResolver().query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, projection2, MediaStore.Video.Media.BUCKET_DISPLAY_NAME+" =?",new String[]{bucket},MediaStore.Video.Media.DATE_ADDED);
        HashSet<String> albumSet = new HashSet<>();
        File file;
    ...
    public void getVideoBuckets(){
        Cursor cursor = getContext().getContentResolver().query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, projection,                 null, null, MediaStore.Video.Media.DATE_ADDED);
...
//        bitmapList.addAll(bitmapListTEMP);
        ...
        {
            bitmapList.add(getVideosBitmap(bucketNames.get(i)));
            bucketSizes.add(getVideosSize(bucketNames.get(i)));
        }
