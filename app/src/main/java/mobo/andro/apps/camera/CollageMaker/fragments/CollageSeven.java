...
public class CollageSeven extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageSeven$3 */
    class C05833 implements Runnable {
        C05833() {
        }

        public void run() {
            CollageSeven.this.setGridPadding(CollageSeven.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageSeven$4 */
    class C05844 extends SimpleOnGestureListener {
        C05844() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageSeven.this.mPopupWindow != null) {
                if (CollageSeven.this.mPopupWindow.isShowing()) {
                    CollageSeven.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageSeven.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageSeven.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageSeven$5 */
    class C05855 implements OnClickListener {
        C05855() {
        }

        public void onClick(View view) {
            CollageSeven.this.mPopupWindow.dismiss();
            CollageSeven.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageSeven$6 */
    class C05866 implements OnClickListener {
        C05866() {
        }

        public void onClick(View view) {
            CollageSeven.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageSeven.this.getStoredBitmap(CollageSeven.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageSeven.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageSeven.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageSeven$7 */
    ...
    public static CollageSeven newInstance(boolean b) {
        CollageSeven fragment = new CollageSeven();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_7, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageSeven.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageSeven$2$1 */
                class C05811 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageSeven.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageSeven.this.getActivity().runOnUiThread(new C05811());
                    }
                }
            });
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C05844());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C05855());
        delete.setOnClickListener(new C05866());
        cancel.setOnClickListener(new C05877());
    }
