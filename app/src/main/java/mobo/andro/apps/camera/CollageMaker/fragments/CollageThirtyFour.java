...
public class CollageThirtyFour extends Fragment implements CollageInterface, OnTouchListener, OnClickListener, TouchCallbackListener {
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyFour$3 */
    class C06553 implements Runnable {
        C06553() {
        }

        public void run() {
            CollageThirtyFour.this.setGridPadding(CollageThirtyFour.this.curPadding);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyFour$4 */
    class C06564 extends SimpleOnGestureListener {
        C06564() {
        }

        public boolean onDoubleTap(MotionEvent e) {
            if (CollageThirtyFour.this.mPopupWindow != null) {
                if (CollageThirtyFour.this.mPopupWindow.isShowing()) {
                    CollageThirtyFour.this.mPopupWindow.dismiss();
                }
                int rawX = (int) e.getRawX();
                int rawY = (int) e.getRawY();
                RelativeLayout main_rel = (RelativeLayout) CollageThirtyFour.this.view.findViewById(R.id.main_rel);
                int[] posXY = new int[2];
                main_rel.getLocationOnScreen(posXY);
                CollageThirtyFour.this.mPopupWindow.showAtLocation(main_rel, 51, rawX + posXY[0], rawY + posXY[1]);
            }
            return true;
        }
    ...
    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyFour$5 */
    class C06575 implements OnClickListener {
        ...
        public void onClick(View view) {
            CollageThirtyFour.this.mPopupWindow.dismiss();
            CollageThirtyFour.this.onGalleryButtonClick(FragmentsManager.REQUEST_GALLERY_ONDOUBLECLICK);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyFour$6 */
    class C06586 implements OnClickListener {
        ...
        public void onClick(View view) {
            CollageThirtyFour.this.mPopupWindow.dismiss();
            Bitmap bitmap = CollageThirtyFour.this.getStoredBitmap(CollageThirtyFour.this.focusedView);
            PhotoEditor.bitmap = bitmap.copy(bitmap.getConfig(), true);
            Intent intentEditor = new Intent(CollageThirtyFour.this.getActivity(), PhotoEditor.class);
            intentEditor.setAction("android.intent.action.MAIN");
            CollageThirtyFour.this.startActivityForResult(intentEditor, FragmentsManager.REQUEST_EDITOR);
        }
    }

    /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyFour$7 */
    class C06597 implements OnClickListener {
        C06597() {
        }
    ...
    public static CollageThirtyFour newInstance(boolean b) {
        CollageThirtyFour fragment = new CollageThirtyFour();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_collage_34, container, false);
    }
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        view.post(new Runnable() {
            public void run() {
                CollageThirtyFour.this.optimizeLayout(view);
            }
        });
        initGd();
    ...
    private void createGLViewForImage(View view, final GLFilterImageView img, final Bitmap bitmap) {
        if (!this.forShowOffOnly) {
            ...
            mainImageView.setSurfaceCreatedCallback(new OnSurfaceCreatedCallback() {
                ...
                /* renamed from: mobo.andro.apps.camera.CollageMaker.fragments.CollageThirtyFour$2$1 */
                class C06531 implements Runnable {
                    ...
                    public void run() {
                        ...
                        img.setFilterConfig(CollageThirtyFour.this.img1.getmCurConfig());
                ...
                public void surfaceCreated() {
                    if (ImageGalleryActivity.selectedBitmaps.size() > 0) {
                        CollageThirtyFour.this.getActivity().runOnUiThread(new C06531());
                    }
                }
            });
    ...
    private void optimizeLayout(View view) {
        ...
        ImageUtils.optimizeView(this.rl8, 0.0f, 0.0f, Dimensions.S_2P, Dimensions.MATCH_PARENT);
        ImageUtils.optimizeView(this.rl9, (float) Dimensions.S_2P, 0.0f, Dimensions.S_2P, Dimensions.MATCH_PARENT);
        view.post(new C06553());
    }
    ...
    public void setGridPadding(int padding) {
        ...
        lp5.setMargins(this.curPadding, this.curPadding / 2, this.curPadding / 2, this.curPadding);
        ...
        this.div5.setX((float) (this.rl8.getWidth() - (this.curPadding / 2)));
    }
                    ...
                    if (view.getId() != R.id.div2 || (view.getY() - ((float) dy)) + ((float) view.getHeight()) > this.rl3.getY() + ((float) this.rl3.getHeight()) || view.getY() - ((float) dy) < this.div1.getY()) {
                        if (view.getId() != R.id.div3 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl5.getX() + ((float) this.rl5.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                            if (view.getId() != R.id.div4 || (view.getX() - ((float) dx)) + ((float) view.getWidth()) > this.rl7.getX() + ((float) this.rl7.getWidth()) || view.getX() - ((float) dx) < 0.0f) {
                                if (view.getId() == R.id.div5 && (view.getX() - ((float) dx)) + ((float) view.getWidth()) <= this.rl9.getX() + ((float) this.rl9.getWidth()) && view.getX() - ((float) dx) >= 0.0f) {
    ...
    private void initGd() {
        this.gdetector = new GestureDetector(getActivity(), new C06564());
    }
    ...
    private void initPopupWindow() {
        ...
        ((TextView) customView.findViewById(R.id.open)).setOnClickListener(new C06575());
        delete.setOnClickListener(new C06586());
        cancel.setOnClickListener(new C06597());
    }
